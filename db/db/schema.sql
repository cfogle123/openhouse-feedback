...
-- Superseded the combined columns above: email and Slack are now tracked
-- (and retried) independently. The original design only marked a reminder
-- "sent" once BOTH channels succeeded, which meant a persistently-failing
-- Slack DM (wrong scope, unmatched email, etc.) caused the working email
-- channel to resend on every single cron check forever, since the slot
-- never reached "fully sent". Splitting these means each channel settles
-- (succeeds, or gives up after 24h) on its own, independent of the other.
